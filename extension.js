...
function pythonPastePro() {
  ...
  const referenceLine = selection.isEmpty
    ? editor.document.lineAt(position.line)
  ...
  vscode.env.clipboard.readText().then((clipboardText) => {
    ...
    const indentUnit = config.get("insertSpaces")
      ? " ".repeat(config.get("tabSize"))
      : "\t";
    const firstLine = originalLines[0].trim();
    const isFirstLineIndentTrigger =
      !firstLine.startsWith("#") && /[:({[]$/.test(firstLine);
    ...
    if (prevLine.text.trim() !== "" && currentIndent.length === 0) {
      ...
      if (!firstLine.startsWith("#") && prevLine.text.endsWith(":"))
        prevLineIndentLevel++;
    }
    let baseIndentLevel = prevLineIndentLevel;
    if (!selection.isEmpty) {
      baseIndentLevel = 0;
      if (selection.start.line === 0 && firstLine.endsWith(":")) {
        baseIndentLevel = 0;
        currentIndentLevel = 0;
      }
    }
    const indentedLines = [indentUnit.repeat(baseIndentLevel) + firstLine];
    ...
    for (let i = 1; i < originalLines.length; i++) {
      ...
      let indentDiff =
        (baseIndent.length - prevBaseIndent.length) / indentUnit.length;
      currentIndentLevel += indentDiff;
      let newIndentLevel =
        currentIndent.length / indentUnit.length + currentIndentLevel;
    ...
    editor.edit((editBuilder) => {
      if (editor.selection.isEmpty) {
        ...
        if (isSingleLineSelection && !isFullLineSelected) {
          ...
          const isSingleLineSelection2 =
            selectionEnd.line - selectionStart.line === 1 &&
            selectionEnd.character === 0;
          const newInsertText =
            currentIndent + insertText + (isSingleLineSelection2 ? "\n" : "");
